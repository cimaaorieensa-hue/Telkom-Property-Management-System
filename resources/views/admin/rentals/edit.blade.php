...
@section('page-subtitle', 'Ubah data penyewa: ' . $rental->nama_penyewa)
            ...
            <form action="{{ route('admin.rentals.update', $rental) }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    <div style="grid-column: 1 / -1;">
                        ...
                        <input type="text" name="nama_penyewa" value="{{ old('nama_penyewa', $rental->nama_penyewa) }}" required style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none;">
                    ...
                    <div>
                        ...
                        <input type="text" name="no_telepon" value="{{ old('no_telepon', $rental->no_telepon) }}" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none;">
                    ...
                    <div>
                        ...
                        <input type="email" name="email_penyewa" value="{{ old('email_penyewa', $rental->email_penyewa) }}" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none;">
                    ...
                    <div style="grid-column: 1 / -1;">
                        ...
                        <select name="property_id" required style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none; background: white;">
                            ...
                            @foreach($properties as $prop)
                                @php
                                    $sisa = $prop->id == $rental->property_id ? $prop->sisa_luas + $rental->luas_sewa : $prop->sisa_luas;
                                @endphp
                                @if($sisa > 0)
                                    <option value="{{ $prop->id }}" {{ old('property_id', $rental->property_id) == $prop->id ? 'selected' : '' }}>
                                        {{ $prop->nama_properti }} (Maks. Area: {{ $sisa }} m²)
                                    </option>
                                @endif
                            @endforeach
                        </select>
                        <small style="color: var(--text-muted); display: block; margin-top: 4px;">Pilih properti yang disewa.</small>
                    ...
                    <div style="grid-column: 1 / -1;">
                        ...
                        <input type="number" step="0.01" name="luas_sewa" value="{{ old('luas_sewa', $rental->luas_sewa) }}" required style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none;">
                        <small style="color: var(--text-muted); display: block; margin-top: 4px;">Masukkan luasan area yang disewa. Tidak boleh melebihi sisa luas properti.</small>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', \Carbon\Carbon::parse($rental->tanggal_mulai)->format('Y-m-d')) }}" required style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none;">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', \Carbon\Carbon::parse($rental->tanggal_selesai)->format('Y-m-d')) }}" required style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none;">
                    ...
                    <div>
                        ...
                        <select name="status_sewa" required style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none; background: white;">
                            <option value="aktif" {{ old('status_sewa', $rental->status_sewa) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="selesai" {{ old('status_sewa', $rental->status_sewa) == 'selesai' ? 'selected' : '' }}>Selesai</option>
                            <option value="dibatalkan" {{ old('status_sewa', $rental->status_sewa) == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                        </select>
                        <small style="color: var(--text-muted); display: block; margin-top: 4px;">Perubahan ke Selesai/Dibatalkan akan me-reset status properti menjadi "Tersedia".</small>
                    ...
                    <div style="grid-column: 1 / -1;">
                        ...
                        <textarea name="catatan" rows="4" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; outline: none; font-family: inherit;">{{ old('catatan', $rental->catatan) }}</textarea>
